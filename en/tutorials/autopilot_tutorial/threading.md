...
On platforms that support real threads the `AP_HAL` for that platform will create a number of threads to support basic operations. For example, in Erle-Brain, the following HAL specific threads are created:
...
Have a look in [Scheduler.cpp](https://github.com/erlerobot/ardupilot/blob/master/libraries/AP_HAL_Linux/Scheduler.cpp) inside each `AP_HAL` implementation to see what threads are created and what the realtime priority of each thread is.
...
An example of a driver specific thread is the ToneAlarm thread in the Linux port. See [ToneAlarmDriver.cpp](https://github.com/erlerobot/ardupilot/blob/master/libraries/AP_HAL_Linux/ToneAlarmDriver.cpp)
...
Secondly, forces the WiFi interface to have the expected `11.0.0.1` IP. Finally, launches the AutoPilot. Depending which vehicle you are using, you will see different executable names: ArduCopter, ArduPlane or APM2rover. It also specifies the telemetry communication protocol, IP and port.
...
It is a table driven scheduler, and each vehicle type has a `AP_Scheduler::Task` table. To learn how it works have a look at the [Scheduler_test.pde sketch](https://github.com/erlerobot/ardupilot/blob/master/libraries/AP_Scheduler/examples/Scheduler_test/Scheduler_test.pde).
...
You should now go and modify the [Scheduler_test](https://github.com/erlerobot/ardupilot/blob/master/libraries/AP_Scheduler/examples/Scheduler_test/Scheduler_test.pde) example and add in your own tasks to run. Try adding tasks that do the following:
...
Go and have a look at the hmc5883 driver in [AP_Compass_HMC5843.cpp](https://github.com/erlerobot/ardupilot/blob/master/libraries/AP_Compass/AP_Compass_HMC5843.cpp) and look for the `_i2c_sem` variable. Look at all the places it is used, and see if you can work out why it is needed.
...
* The `_shared_data` structure in [AP_InertialSensor_MPU9250.cpp](https://github.com/erlerobot/ardupilot/blob/master/libraries/AP_InertialSensor/AP_InertialSensor_MPU9250.cpp)

* The ring buffers used in numerous places. A good example is [DataFlash_File.cpp](https://github.com/erlerobot/ardupilot/blob/master/libraries/DataFlash/DataFlash_File.cpp)
